...
## 30a. Produktklarstellung: Zentrale manuelle Kosten-Inbox (Arbeitsblock 3, 23.07.2026)

Der Academy-MVP besitzt **eine zentrale, manuelle Kosten-Inbox**: Der Mensch führt Belege über eine Upload-Fläche zu (Zuführung ist manuell). Die Verarbeitung danach ist ein regelbasierter, zustandsabhängiger Agent, der pro Eingang einen Ausführungsplan erstellt, notwendige Prüfwerkzeuge wählt, ungeeignete Schritte überspringt und anhand von Evidenz und vorhandenem Bestand über die nächste Aktion entscheidet (Wahrnehmen, Planen, Werkzeuge ausführen, Bewerten, Handeln, Erklären, Erinnern; siehe Abschnitt 7 und 32). Fokus des MVP: geschäftliche Kosten mit Beleg oder Erfassungsnachweis. Spätere Eingangsadapter (Mail-Wache, mobile Teilen-Funktion, Bankintegration, Claude-Chat, Optifyx-Anbindung) sind dokumentiert (Abschnitt 6), aber in diesem Slice nicht implementiert.
...
2. **Planen:** ein echter Ausführungsplan (`belegwaechter/planen.py`) legt je nach Quellenklasse fest, welche Werkzeuge laufen — Stufe A bekommt vollständige Extraktion plus Checkliste plus Bestandsabgleich; Stufe B bekommt nur Klassifizierung plus sofortige Review-Aufgabe (Extraktionswerkzeuge werden bewusst übersprungen, da das Gate nicht bestanden ist). Neue Evidenz während der Verarbeitung (Lesefehler, Dublette, unvollständige Checkliste) löst eine protokollierte Planrevision aus, die zum Beispiel das Abo-Radar nachträglich deaktiviert. Der Executor fragt ausschließlich diesen Plan ab, er verzweigt nirgends ein zweites Mal eigenständig anhand der Stufe.
